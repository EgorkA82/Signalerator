...
Самым важным обновлением программы стало добавление полноценной генерации сигнала. Стали доступны 4 вида сигнала:
...
В программе ведётся "статус" сигнала - число от 0 до 1, обозначающее прогресс сигнала в данном периоде. Если прогресс равен 0.5, то половина периода сигнала прошла. Используя эту переменную (обозначим её как `x`), мы можем определить значение сигнала:

* Прямоугольный - $(x \lt 0.5 \Rightarrow 0) \iff (x \geq 0.5 \Rightarrow 1)$
* Треугольный - $(x \lt 0.5 \Rightarrow 2x) \iff (x \geq 0.5 \Rightarrow 2(-x + 1))$
* Синусоида - $\sin(2 \pi x + 1)$
* Произвольный - $\int_{0}^{1} f(x)$
